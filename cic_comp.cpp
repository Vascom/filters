...
int main(int argc, char *argv[])
{
    ...
    const double F_stop_incr = 0.5; //Increment used if initial frequency stop level can't matched
    const double q_stop_min = atof(argv[3]);  //Required stop level (initial), dB
    const double q_stop_max = atof(argv[4]);  //Required stop level (max), dB
    const double q_stop_incr = 1;   //Increment used if initial stop level can't matched
    const double q_pass = -40;      //Min pass level, dB
